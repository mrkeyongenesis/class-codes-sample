...
> *"Tell the "* note explaining **why** the command matters and a
...
**Tell the :** `lsblk` is the single most important "where am I" command in
...
**Tell the :** RAM is fast but limited. Swap is disk space the kernel uses to
...
**Tell the :** type code `82` flags the partition's *intended* purpose. The
...
**Tell the :** `dd` copies blocks. `if` = input file (`/dev/zero` = endless
...
**Tell the :** Watch the **Used** column climb on the watch screen as the kernel
...
**Tell the :** **Higher number = used first.** So `3 > 1 > -1 > -3`. If you have
...
**Tell the :** The six fields are *device, mountpoint, type, options, dump,
...
**Tell the :** Plain partitions are rigid — once `/data` is 100 GB it's stuck at
...
**Tell the :** the `8e` type isn't strictly required for LVM to work, but it
...
**Tell the :** `pvcreate` writes an LVM label + metadata area to each partition.
...
**Tell the :** The VG is now a single storage *pool*. Its size = sum of its PVs.
...
**Tell the :** `-L 200M` sizes by capacity; you can also size by extents with
...
**Tell the :** Notice the mounted name shows as `/dev/mapper/my_first_vg-my_first_lv`.
...
**Tell the :** the *container* (LV) is bigger, but the *filesystem* inside
...
**Tell the :** if you remove things out of order (e.g. `pvremove` a disk still in
...
**Tell the :** Red Hat also ships **Stratis**, which layers pools + filesystems
